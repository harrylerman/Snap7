...
if __name__=="__main__":
    ...
    try:
        ...
        autotune=False
    logger.info("# (3c) Autotune: %s"%(autotune))
    ...
    logger.info("# (11) Reboot RPi when drive becomes full: %s"%(reboot_when_full))
    try:
        have_mux=config_file.getboolean("albatros2","have_mux")
    except:
        have_mux=False
    logger.info("# (12) Expecting connection to MUX: %s"%(have_mux))
    logger.info("##########################################################################")

    sock=socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    sock.settimeout(10) # setting a timeout to prevent hanging forever on sock.recvfrom_into(packet, bytes_per_packet) line after overheat.
    try:
            ...
            for drive in drives:
                ...
                if number_of_files<1:
                    ...
                    if have_mux:
                        supertools.mark_drive_full()
                    if reboot_when_full:
                        # should nicely unmount current drive here before rebooting
                        #os.system("sudo umount "+repr(drive_path))
                        #os.system("sudo udisksctl power-off -b "+repr(drive["Device"]))
                        #muxtools.init_mux()
                        #muxtools.free_drive(repr(drive["Device"]))
                        #logger.info("Safely unmounted full drive mounted on "+repr(drive_path))
                        logger.info("Rebooting")
                        os.system("sudo reboot")
            drives_full=True

    if drives_full:
        # if not reboot_when_full:
            # logger.info("reboot_when_full is false so will not mount another drive")
        ...
        logger.info('No drives mounted! Stopping dump_baseband.py')
        raise RuntimeError('No drives mounted! Stopping dump_baseband.py')
