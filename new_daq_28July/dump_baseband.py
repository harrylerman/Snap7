...
if __name__=="__main__":
    ...
    # also prints log messages as stdout, comment these lines out if you don't like it
    stdout_logger=logging.StreamHandler(sys.stdout)
    stdout_logger.setLevel(logging.INFO)
    logger.addHandler(stdout_logger)


    # getting relevant params from config file & saving to log
    logger.info("##########################################################################")
    ...
    try:
        autotune=config_file.getboolean("albatros2","autotune")
    except:
        autotune=False
...
#    logger.info("# (6) Drives full: %r"%(drives_full))
    ...
    logger.info("# (6) Drive safety percentage: %.2f"%(drive_safety))
    ...
    logger.info("# (8) Baseband directory name: %s"%(dump_baseband_directory_name))
    logger.info("# (9) Log directory: %s"%(baseband_log_dir))

    try:
        reboot_when_full=config_file.getboolean("albatros2","reboot_when_full")
    except:
        reboot_when_full=False
    logger.info("# (10) Reboot RPi when drive is filled: %s"%(reboot_when_full))

    try:
        have_mux=config_file.getboolean("albatros2","have_mux")
    except:
        have_mux=False
    logger.info("# (11) Expecting connection to MUX: %s"%(have_mux))

    logger.info("##########################################################################")


    #======================= setting up SNAP comms  =======================#
    ...
    #======================= figuring out drive situation  =======================#

    if have_mux:
        if supertools.drives_full() == None: # all MUX drives are full
            logger.info("All MUX drives full, not writing baseband, pausing here")
            while True: # hanging so that killing script doesn't mess with albaboss
                pass 
            
    drives=albatros_daq_utils.list_drives_to_write_too("BASEBAND")
    logger.info("Found these drive/s")
    logger.info("%-17s %-17s %-17s %-17s %-5s%% %-s"%("Device", "Total", "Used", "Free", "Use ", "Mount"))

    for drive in drives: # logging drives
    ...
    if len(drives)==0: # no drives are mounted for some reason
        logger.info("No drives found. Check if drives are connected and mounted")
        raise RuntimeError('No drives mounted! Stopping dump_baseband.py. I will come for you in your sleep.')

    #======================= writing baseband =======================#
    for drive in drives:

        drive_path=drive["Mounted on"]
        number_of_files=albatros_daq_utils.num_files_can_write(drive_path, drive_safety, file_size) # checks how many files can write
        ...
        if number_of_files>=1:
            ...
            for i in range(number_of_files): # writing baseband files
        ...
        # done iterating thru num files, so drive is full
        logger.info("Drive "+repr(drive_path)+" is full.")
    ...
    #hanging so that terminating dump_baseband doesn't mess up other running scripts
    logger.info("All drives are full, hanging here")
    while True:
        pass
